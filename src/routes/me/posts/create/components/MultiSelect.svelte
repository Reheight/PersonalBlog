<script>
  ...
  <div class="multiselect mb-1 z-[2] px-1" class:readonly>
    <div class="tokens" class:showOptions on:click={handleTokenClick}>
      {#each Object.values(selected) as s}
        <div class="token flex flex-row items-center justify-between m-1 px-4 py-1 bg-orange-300 hover:bg-orange-400 ease-in-out transition-all transform-gpu rounded-lg" data-id="{s.value}">
          <span>{s.name}</span>
          {#if !readonly}
