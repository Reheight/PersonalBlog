	...
	<div class="w-full p-1 bg-orange-300 rounded-sm shadow-sm text-white">
		<div class="text-gray-800 flex flex-col justify-center m-2">
			...
			<div class="flex flex-row justify-between items-center text-gray-600">
				<span
					>By, <a href={`/profile/@${data.author.username}`}
						>{data.author.name} (@{data.author.username})</a
					></span
				>
				<span
		...
		<div
			class="m-2 p-2 border-2 border-orange-200 bg-gray-200 rounded-md relative overflow-hidden text-gray-900"
		>
			<Viewer {plugins} value={data.content} />
		</div>
		<div
			class="flex flex-row gap-1 items-center justify-start bg-gray-400 rounded-sm p-1 overflow-x-auto m-2"
		>
		...
		{#if $page.data.member}
			...
			>
				<form
					action="?/comment"
					class="flex flex-row w-full overflow-hidden border-2 border-orange-200"
					method="post"
					use:enhance
		...
		<div class="m-2 bg-orange-200 rounded-sm p-1 text-gray-700">
			{#if data.comments}
				{#each data.comments as comment, i}
					<div
						class="flex flex-row p-1 mt-3 bg-orange-100 shadow m-1 rounded-sm items-center relative"
						id={`${comment.id}`}
					>
						{#if comment.author.id === $page.data.member?.id || data.author.id === $page.data.member?.id || $page.data.member?.role === 'ADMINISTRATOR'}
							<form action="?/deletecomment" method="post" id={`${i}`} use:enhance>
								...
								<button
									type="submit"
									title="Delete"
									class="absolute border-4 border-orange-200 -top-1.5 -right-1 text-white bg-red-300 p-1 text-sm rounded-xl px-1.5 eas-in-out transition-all transform-gpu hover:scale-110 hover:bg-red-500"
									><Fa icon={faTimes} /></button
								>
							</form>
						{/if}
						<div
							class="py-1 px-2 rounded-sm bg-orange-200 mr-2 w-[200px] sm:w-[300px] lg:w-[350px] m overflow-hidden"
						>
							<div class="flex flex-row justify-between items-center font-montserrat text-gray-900">
								<a
									href={`/profile/@${comment.author.username}`}
									class=" text-base whitespace-nowrap truncate">{comment.author.name}</a
								>
								{#if comment.author.id === data.author.id}<span
										class="ml-2 px-1 py-[1.5px] text-xs flex justify-center items-center rounded-sm bg-gray-600 text-gray-100"
										>OP</span
									>{/if}
							</div>

							<p class="text-sm whitespace-nowrap truncate font-montserrat">
						...
						<p class="flex flex-row self-start font-robotomono text-sm py-2 mr-3">
							{comment.content}
						</p>
					</div>
				{/each}
